...
export const MINIMUM_ADVISOR_COUNT = 2;

export function validateInitialCouncilSelection({ councilStatus, agentIds } = {}) {
  const selectedCount = uniqueIds(agentIds).length;
  const minimum = councilStatus === 'draft' ? MINIMUM_ADVISOR_COUNT : 1;
  return {
    allowed: selectedCount >= minimum,
    minimum,
    selectedCount,
  };
}
...
export function normalizeExecuteResponse(input = {}) {
  return {
    ...
    affectedAdvisorIds: Array.isArray(input.affectedAdvisorIds) ? input.affectedAdvisorIds : [],
