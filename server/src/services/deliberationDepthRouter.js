...
export function routeDeliberationDepth(question) {
  ...
  if (HIGH_STAKES.test(normalized)) {
    return { depth: 'standard', reason: '涉及较高影响或专业风险，不能用即时建议替代完整判断' };
  }
  if (normalized.length <= 40 && QUICK_EVERYDAY.test(normalized)) {
    return { depth: 'quick', reason: '低风险、可逆的即时日常选择' };
  }
  return { depth: 'standard', reason: '需要拆解取舍并核对信息' };
}
