...
| 三层提示词 | ✅ | identity/methodology/deliverable（后端权威源，前端同步12个智囊persona做降级） |
...
| 任务 | 说明 | 状态 |
|------|------|------|
| **persona/prompt 前后端统一收敛到后端** | 前端`AGENT_PERSONAS`已同步后端`agentPool.js`的12个智囊persona字段，后端为权威源，前端只做本地降级 | ✅ 完成 |
| 验证完整推演流程无报错 | 本地跑一遍 input→final 全流程 | ✅ 通过 |
...
| 3 | Prompt前后端双份维护 | ✅ 已解决 | 后端agentPool.js为权威源，前端AGENT_PERSONAS同步12个智囊persona做降级 |
