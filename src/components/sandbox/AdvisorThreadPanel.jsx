...
import { uniqueRoles } from '../../game/historyPresentation.js';
...
export default function AdvisorThreadPanel({ sessionId, advisors = [], projection, onQuestion, pending = false, focusedAdvisorId = '', onFocusAdvisor, history = {}, variant = 'inline' }) {
  const council = useMemo(() => uniqueRoles(advisors), [advisors]);
  ...
  const projectedAgents = useMemo(() => projection?.agents || {}, [projection?.agents]);
  ...
  useEffect(() => {
    if (!sessionId || council.length === 0 || focusedAdvisorId || threads.length > 0) return;
    const openingThread = createAdvisorThread(sessionId, council.map((advisor) => advisor.id), {
  ...
  }, [council, focusedAdvisorId, sessionId, threads.length]);

  useEffect(() => { saveAdvisorThreads(sessionId, threads); }, [sessionId, threads]);

  const activeThread = threads.find((thread) => thread.id === activeThreadId) || null;
  useEffect(() => { setMemberIds(activeThread?.memberIds || []); }, [activeThreadId, activeThread?.memberIds, activeThread?.updatedAt]);
  ...
  useEffect(() => {
    ...
    const advisor = council.find((item) => item.id === focusedAdvisorId);
    if (!advisor) return;
    const id = privateThreadId(advisor.id);
    setThreads((previous) => previous.some((thread) => thread.id === id)
      ? previous
      : [...previous, createAdvisorThread(sessionId, [advisor.id], { id, title: advisorName(advisor) })]);
    setActiveThreadId(id);
  }, [focusedAdvisorId, sessionId, council]);

  const createGroup = () => {
    const initial = memberIds.length > 0 ? memberIds : council.map((advisor) => advisor.id);
  ...
  const matchedMentions = mentionMemberIds(draft, council);
  ...
  const openingStatements = useMemo(() => {
    ...
    return council.flatMap((advisor) => {
  ...
  }, [activeThread?.kind, council, history, projectedAgents]);
  ...
  return (
    <section className={`advisor-thread-panel advisor-thread-panel--${variant}`} aria-label="智囊对话">
      <header>
        <span><strong>{activeThread?.title || (variant === 'workbench' ? '本局智囊对话' : '智囊对话')}</strong><small>{council.length} 位正式智囊 · 私聊与 @ 不改变本局阵容</small></span>
      ...
      {activeThread ? <>
        <div className="advisor-thread-panel__thread-bar">
          ...
          <details className="advisor-thread-panel__roster"><summary>成员 {effectiveMembers.length}/{council.length}</summary>
            <nav className="advisor-thread-panel__people" aria-label="打开智囊单聊">
              {council.map((advisor) => <button type="button" key={advisor.id} data-active={activeThreadId === privateThreadId(advisor.id)} onClick={() => openPrivate(advisor)}><i>{advisor.trigram || advisor.icon || '◦'}</i><span>{advisorName(advisor)}</span></button>)}
            </nav>
            <div className="advisor-thread-panel__members">
              <strong>下一条发给</strong>
              {council.map((advisor) => <button type="button" key={advisor.id} aria-pressed={effectiveMembers.includes(advisor.id)} onClick={() => setMemberIds((previous) => previous.includes(advisor.id) ? previous.filter((id) => id !== advisor.id) : [...previous, advisor.id])}>{effectiveMembers.includes(advisor.id) ? '✓ ' : '+ '}{advisorName(advisor)}</button>)}
            </div>
          </details>
        </div>
        <div className="advisor-thread-panel__messages">
          ...
          {(activeThread.messages || []).length === 0 && activeHistory.length > 0 && <details className="advisor-thread-panel__round-record"><summary>此前发言 · {activeHistory.length} 条</summary>{activeHistory.map((text, index) => <article key={`${activeAdvisorId}:history:${index}`} data-role="advisor"><span>{advisorName(council.find((advisor) => advisor.id === activeAdvisorId))}</span><p>{text}</p></article>)}</details>}
          ...
          {(activeThread.messages || []).map((message) => <article key={message.id} data-role={message.role}><span>{message.authorName}</span><p>{message.text}</p>{message.role === 'user' && <small>{message.status === 'failed' ? '发送失败，请重新发送' : `发送给 ${message.memberSnapshot.map((id) => advisorName(council.find((advisor) => advisor.id === id))).join('、')}`}</small>}</article>)}
        ...
        <div className="advisor-thread-panel__composer">
          <div className="advisor-thread-panel__draft">
            <div className="advisor-thread-panel__mention-shortcuts"><span>@邀请</span>{council.filter((advisor) => !effectiveMembers.includes(advisor.id)).map((advisor) => <button type="button" key={advisor.id} onClick={() => setDraft((value) => `${value}${value && !/\s$/.test(value) ? ' ' : ''}@${advisorName(advisor)} `)}>@{advisorName(advisor)}</button>)}</div>
