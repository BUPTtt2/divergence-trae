...
- 当前 `/sandbox` 已切到 Session/Event 驱动的 Agent 主链，生产 PostgreSQL 已接通；用户全链路、Vercel + Surge 与真实 iPad Safari 终验未完成，因此仍不能宣称生产级。
- 目标是“东方仪式语言 + 可验证 Agent Runtime + 智囊资产 + 决策账本 + 结果校准”。
- 旧版的推荐、选择、市集、铸造和逐位对话要迁移到新 Runtime，不恢复双轨。
- 当前执行阶段：**14 · 全产品智囊生态与决策闭环**。

## 01 · 权威设计

- [Sandbox 生产级 Multi-Agent 系统设计与现状审计](规格/00-Sandbox生产级多Agent系统设计与现状审计.md)
- [全产品智囊生态与决策闭环设计](规格/14-全产品智囊生态与决策闭环设计.md)
...
| 07 | 事件实况台体验改造 | Session 立即返回，真实过程可读，小窗/iPad 可完整体验 | 本地完成，待生产持久层（2026-08-08） |
| 08 | 自适应推演深度 | 简单问题不滥用 Multi-Agent，复杂问题不被压缩 | 本地完成（2026-08-08） |
| 09 | 小窗口与 iPad 展陈适配 | 关键过程、信息和操作在目标尺寸均可达 | 浏览器尺寸验收完成，待真实 iPad Safari（2026-08-08） |
| 10 | Vercel + Surge 部署基线 | 生产主链可恢复、可重复体验 | PostgreSQL 已接通，待本地验收后部署（2026-08-08） |
| 11 | 用户信息案卷与开演门禁 | 已知/未知/记忆可见，用户确认后才能执行 | 本地完成（2026-08-08） |
| 12 | 可中断多 Agent 对话 | Agent 可追问，用户可补充、纠正、追问、暂停 | 本地实现完成，待用户验收（2026-08-08） |
| 13 | 自适应编排与伴行助手 | 回答驱动重新规划，智囊分派可解释，交互不遮挡主舞台 | Vercel + Surge 生产候选已发布；待更换模型密钥及线上实机验收（2026-08-09） |
| 14 | 全产品智囊生态 | 推荐—选择—推演—收藏闭环贯通 | 第一闭环已接通：案卷后择智、真实阵容执行与恢复；真实市集仍待替换（2026-08-09） |
| 15 | 决策账本与结果校准 | 3/7/30/90 天结果可回写并校准 Agent | 14 完成后实施 |
| 16 | Agent Studio | Agent Contract 可测试、评估、发布和回滚 | 产品门槛通过后设计 |
...
- Stage 11 设计：[用户信息案卷与开演门禁](规格/11-用户信息案卷与开演门禁设计.md)
- Stage 11 计划：[用户信息案卷与开演门禁实施计划](计划/11-用户信息案卷与开演门禁实施计划.md)
- Stage 12 设计：[可中断 Multi-Agent 对话与单一交互面设计](规格/12-可中断多Agent对话与单一交互面设计.md)
- Stage 12 计划：[可中断 Multi-Agent 对话实施计划](计划/12-可中断多Agent对话实施计划.md)
- Stage 13 设计：[自适应编排与伴行助手系统设计](规格/13-自适应编排与伴行助手系统设计.md)
- Stage 13 计划：[自适应编排与伴行助手实施计划](计划/13-自适应编排与伴行助手实施计划.md)
- Stage 14 设计：[全产品智囊生态与决策闭环](规格/14-全产品智囊生态与决策闭环设计.md)
- Stage 14 计划：[智囊会选择与全产品闭环实施计划](计划/14-智囊会选择与全产品闭环实施计划.md)
- 复赛验收：[复赛本地验收与生产监测清单](验收/01-复赛本地验收与生产监测清单.md)
...
- Stage 07 设计：[推演体验与事件动画设计](规格/07-推演体验与事件动画设计.md)
- Stage 08 设计：[自适应推演深度与问题路由设计](规格/08-自适应推演深度与问题路由设计.md)
- Stage 09 设计：[小窗口与 iPad 展陈适配设计](规格/09-小窗口与iPad展陈适配设计.md)
- Stage 10 设计：[Vercel 后端与 Surge 前端部署基线](规格/10-Vercel后端与Surge前端部署基线.md)
- Stage 07 实施：[事件实况台体验改造实施计划](计划/07-事件实况台体验改造实施计划.md)

### Stage 14 当前证据与边界

1. 已真实盘点旧版智囊阁、三类择智、市集、五步铸造和推演内跳转，没有仅凭截图重画。
2. 案卷确认后不再直接执行，进入独立 Council 门禁；演推荐、我的智囊、市集候选和去铸造均可见，用户至少选择一位后才可开辩。
3. 用户确认的 `selectedAgentIds` 写入 Session 和 `COUNCIL_CONFIRMED` 事件；执行、事件投影与刷新恢复均以该阵容为准，不再恢复成全部候选。
4. “要不要吃饭”浏览器实测会先询问身体状态、上一餐和当前目标，并展示编排总管为何分派养生与心禾，不再假定用户饥饿。
5. 1024×768 与 820×1180 浏览器尺寸已完成 Council 交互验收；仍需真实 iPad Safari 检查软键盘、安全区与现场网络。
6. 当前市集仍使用旧版 localStorage 样例与订阅逻辑，尚未达到生产真实性；Stage 14 退出前必须接入共享池、真实发布/订阅和版本化 AdvisorAsset。
...
## 11 · Stage 07–09 本地验收证据

1. Session 与规划拆分：`/start` 可先返回 Session，前端接通 SSE 后再调用 `/plan`；用户不再等待模型规划完成后才看到第一条反馈。
2. 事件实况：界面公开显示会话、规划、任务、智囊、信息缺口、查证、证据、分歧、重规划和收束，不展示私有思维链。
3. 深度路由：“要不要吃饭”被判为低风险快推演，约 0.1 秒形成 2 项任务、1 位养生智囊和 1 个真正影响建议的追问；辞职、投资、法律、用药等高影响问题仍走标准推演。
4. 中央交互：待答问题直接驱动中央追问层，不再因传输状态稍慢而只剩空画面；Framer Motion 与 CSS 位移不再互相覆盖，小窗横屏中追问保持在实况侧栏右侧。
5. 自适应：桌面为事件侧栏；iPad 竖屏与手机为有高度上限的底部实况抽屉；中央交互层为抽屉预留空间；1024×768 保持 320px 受限侧栏；控件至少 44px，并支持 reduced/off。
6. 自动验证：服务端 138/138、前端 54/54；production build 成功并转换 1076 个模块；定向 oxlint 0 error，保留 `Game.jsx` 7 条既有未使用项 warning。
7. 浏览器实测：本地输入“要不要吃饭”后可见建会话、开始规划、快推演理由、任务、智囊与追问；回答“有点饿，五小时前吃的”后进入单智囊推演，而非再次拉起多轮无关澄清。
8. 生产边界：Vercel 当前未配置 `DATABASE_URL`，所以尚未发布本轮 Session/SSE 拆分；无 PostgreSQL 时跨请求可能丢 Session，不能作为比赛稳定链接。
