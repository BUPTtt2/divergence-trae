...
- 当前 `/sandbox` 已切到 Session/Event 驱动的 Agent 主链，但生产 PostgreSQL 与真实 iPad Safari 终验未完成，因此仍不能宣称生产级。
- 目标是“东方仪式语言 + 可验证 Agent Runtime + 决策账本 + 结果校准”。
- 不先增加新动画、智囊数量或外围页面。
- 当前执行阶段：**12 可中断多 Agent 对话正在实现；10 仍需接入 PostgreSQL 后才能发布 Vercel + Surge 比赛版本**。
...
| 10 | Vercel + Surge 部署基线 | 生产主链可恢复、可重复体验 | 阻塞于生产 `DATABASE_URL`（2026-08-08） |
| 11 | 用户信息案卷与开演门禁 | 已知/未知/记忆可见，用户确认后才能执行 | 本地完成（2026-08-08） |
| 12 | 可中断多 Agent 对话 | Agent 可追问，用户可补充、纠正、追问、暂停 | 开发中（2026-08-08） |
| 13 | 决策账本与结果校准 | 3/7/30/90 天结果可回写并校准 Agent | 10 完成后设计 |
| 14 | Agent Studio | Agent Contract 可测试、评估、发布和回滚 | 产品门槛通过后设计 |
